  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1)
	{
		if (petWarning)
		{
			...
			warningInfo[0] = 0;
			ssd1306_WriteString("For the past 5 days your pet", Font_7x10, White);
			if (warningInfo[1] == -1)
			{
				ssd1306_SetCursor(2,y);
		...
		else if (foodFresh < 0 || waterFresh < 0)
		{
			...
			if (is_press && keyPressed == 35)
			{
				uint8_t cmd_update_tank[32];
				cmd_update_tank[0]='*';
				cmd_update_tank[31]='\0';
				HAL_UART_Transmit(&huart2, cmd_update_tank, 32, 500);
				is_press = 0;
			}
		}
		else
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if (char_buffer[0]== '$' && (char_buffer[1]=='T' || char_buffer[1]=='F')){ // receive a warning array
		if (char_buffer[1]=='T'){ // need to change food in food tank
			foodFresh=-1;
		}
		if (char_buffer[2]=='T'){ // need to change water
			waterFresh=-1;
		}
		if (char_buffer[3]=='T'){ // the pet is eating too much
			petWarning=1;
			warningInfo[0]=1;
		}
		if (char_buffer[4]=='T'){ // the pet is eating too little
			petWarning=1;
			warningInfo[0]=-1;
		}
		if (char_buffer[5]=='T'){ // the pet is drinking too little
			petWarning=1;
			warningInfo[1]=-1;
		}
	}
	else if (char_buffer[0]== '$'){ // receive a data array //总共会传过来5个data array，是前五天的数据
		char tp[4];
		tp[3]='\0';
		char tp2[6];
		tp2[5]='\0';
		...
		float temp1; // 每个传过来的array里的数据都暂时放在这5个float里了
		float humd1;
		float water1;
		float food1;
		float pet_weight;
		for (int i = 0; i < 3 ; i++) {
			tp[i] = char_buffer[i+1];
		}
		...
		// 可在这里把 temp1，humd1等五个float放在history里
		...
		history_food[0] = food1;
		hsitory_water[0] = water1;
		history_pet[0] = pet_weight;
		history_temp[0] = temp1;
		history_hum[0]= humd1;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	// setup a timer interrupt every 6 hour to determine whether or not an automatic feeding is needed
	if (htim->Instance == htim5.Instance){
		...
		if (timer5_food_count > 1 && food_weight < 5)
		{
			release_food();
			timer5_food_count = 0;
		}
		if (timer5_water_count > 1 && water_weight < 5) // maybe change to container's weight
		{
			release_water();
